(function () {
    app.controller('HomeInvoiceList', function ($scope, $http, $filter) {
        ...
        $scope.GetInvoiceSalesBySalesId = function (salesId) {
            ...
            $http.get('/Home/GetInvoiceBySalesId', { params: { salesId: salesId } })
                .then(function (response) {
                    ...
                    $scope.Sale.DiscountPercentage = $scope.InvoiceCart[0].DiscountPercentage;

                    $scope.Sale.VatPercentage = $scope.InvoiceCart[0].VatPercentage;
        ...
        $scope.CalculateDiscount = function () {
            $scope.Sale.DiscountAmount = ($scope.Sale.Subtotal * $scope.Sale.DiscountPercentage) / 100;
        }
        $scope.CalculateVat = function () {
            $scope.Sale.VatAmount = (($scope.Sale.Subtotal - $scope.Sale.DiscountAmount) * $scope.Sale.VatPercentage) / 100;
